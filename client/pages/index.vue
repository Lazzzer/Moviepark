<template>
  ...
  >
    <section class="relative mb-10 text-center">
      <div>
        <svg class="w-auto h-11 sm:h-15 md:h-17" viewBox="0 0 1024 205" fill="none" xmlns="http://www.w3.org/2000/svg">
...
export default {
  ...
  async asyncData ({ $axios, env, error }) {
    try {
      const trending = await $axios.get('/tmdb/trending')
      const nextTheaters = await $axios.get('/tmdb/nextTheaters')
